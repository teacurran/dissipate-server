...
public class DissipateServiceImpl implements DissipateService {
  ...
  public static final String ERROR_TOKEN_INVALID = "auth.error.token.invalid";

  public static final String ERROR_TOKEN_CONSUMED = "auth.error.token.consumed";
  ...
  public Uni<ValidateSessionResponse> validateSession(ValidateSessionRequest request) {
    ...
    return SessionValidation.findBySidToken(request.getSid(), request.getOtp()).onItem().transformToUni(sv -> {
      if (sv == null) {
        throw new ApiException(Status.NOT_FOUND, ERROR_TOKEN_INVALID, i18n.getString(ERROR_EMAIL_INVALID));
      }
